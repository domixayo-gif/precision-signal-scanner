...
ASSET_BASES = ['BTC', 'ETH', 'SOL', 'BNB', 'ADA', 'TRX', 'LINK', 'TON', 'AVAX', 'DOGE', 'DOT', 'LTC', 'POL']
MAIN_SECONDS = 300
ENTRY_SECONDS = 60
MIN_SCORE = 80
REQUEST_TIMEOUT = 20
...
def send_message(text):
    ...
    try:
        for start in range(0, len(text), 3900):
            tg('sendMessage', {'chat_id': TELEGRAM_CHAT_ID, 'text': text[start:start + 3900], 'parse_mode': 'HTML', 'disable_web_page_preview': 'true'})
        return True
...
def api_get(url, params=None):
    response = requests.get(url, params=params, timeout=REQUEST_TIMEOUT, headers={'User-Agent': 'precision-signal-scanner/3.1'})
    response.raise_for_status()
...
def process_commands(data):
    ...
    changed = False
    try:
        updates = tg('getUpdates', {'offset': int(data.get('offset', 0)) + 1, 'limit': 100, 'timeout': 1}) or []
    except Exception as exc:
    ...
    for update in updates:
        ...
        if command in ('/win', '/loss'):
            if len(parts) != 2:
                send_message('⚠️ Use <code>' + command + ' SIGNAL-ID</code>')
                continue
            wanted = parts[1]
            found = None
            for item in data['signals']:
                if item.get('id') == wanted:
                    ...
                    break
            if found is None:
                send_message('❌ Signal not found: <code>' + html.escape(wanted) + '</code>')
                continue
            if found.get('result', 'PENDING') != 'PENDING':
                send_message('⚠️ <code>' + html.escape(wanted) + '</code> is already <b>' + html.escape(str(found.get('result'))) + '</b>.')
                continue
            result = 'WIN' if command == '/win' else 'LOSS'
            found['result'] = result
            found['result_time'] = iso_now()
            changed = True
            send_message('✅ <b>RESULT RECORDED</b>\nSignal: <code>' + html.escape(wanted) + '</code>\nResult: <b>' + result + '</b>')
            continue
...
def get_candles(product_id, granularity, limit=220):
    raw = api_get(COINBASE_BASE + '/products/' + product_id + '/candles', {'granularity': granularity})
    candles = []
    for row in raw:
...
def true_ranges(candles):
    ...
    for i, candle in enumerate(candles):
        ...
        tr[i] = max(candle['high'] - candle['low'], abs(candle['high'] - previous_close), abs(candle['low'] - previous_close))
    return tr
...
def adx_dmi(candles, period=14):
    ...
    for i in range(period, n):
        ...
        minus_di[i] = mdi
        denominator = pdi + mdi
        dx = 0.0 if denominator == 0 else 100.0 * abs(pdi - mdi) / denominator
        dx_values.append(dx)
...
def bullish_confirmation(candles):
    ...
    current = candles[-1]
    previous = candles[-2]
    strong = direction(current) == 'BULL' and body_ratio(current) >= 0.55 and (current['close'] > previous['close'])
    engulfing = direction(previous) == 'BEAR' and direction(current) == 'BULL' and (current['open'] <= previous['close']) and (current['close'] >= previous['open'])
    reclaim = current['close'] > previous['high'] and previous['low'] <= candles[-3]['low']
...
def bearish_confirmation(candles):
    ...
    strong = direction(current) == 'BEAR' and body_ratio(current) >= 0.55 and (current['close'] < previous['close'])
    engulfing = direction(previous) == 'BULL' and direction(current) == 'BEAR' and (current['open'] >= previous['close']) and (current['close'] <= previous['open'])
    reclaim = current['close'] < previous['low'] and previous['high'] >= candles[-3]['high']
    return strong or engulfing or reclaim
...
def analyze_asset(symbol, product_id):
    ...
    if len(candles5) < 100 or len(candles1) < 100:
        return {'signal': 'NO TRADE', 'score': 0, 'reason': 'Insufficient data'}
    ...
    i5 = len(candles5) - 1
    i1 = len(candles1) - 1
    required = [e20[i5], e50[i5], rsi5[i5], hist5[i5], atr5[i5], pdi[i5], mdi[i5], adx[i5], e9[i1], e21[i1], rsi1[i1], hist1[i1]]
    if any((x is None for x in required)):
        return {'signal': 'NO TRADE', 'score': 0, 'reason': 'Indicators not ready'}
    price = close5[i5]
    price1 = close1[i1]
    ema20 = e20[i5]
    ema50 = e50[i5]
    ...
    if support is None or resistance is None:
        return {'signal': 'NO TRADE', 'score': 0, 'reason': 'Levels unavailable'}
    bull_trend = price > ema20 > ema50
    bear_trend = price < ema20 < ema50
    bull_ema_slope = ema20 > prev_ema20 and ema50 > prev_ema50
    bear_ema_slope = ema20 < prev_ema20 and ema50 < prev_ema50
    ...
    recent = candles1[-6:]
    bull_pullback = any((c['low'] <= ema9 * 1.0015 for c in recent[:-1]))
    bear_pullback = any((c['high'] >= ema9 * 0.9985 for c in recent[:-1]))
    ...
    if bull_entry_trend:
        bull_score += 6        bull_reasons.append('1M EMA alignment')
    if bear_entry_trend:
        bear_score += 6
    ...
    bull_ready = all([bull_trend, bull_ema_slope, bull_structure, bull_dmi, strong_trend, bull_momentum, bull_rsi, bull_entry_trend, bull_entry_momentum, bull_entry_rsi, bull_candle, bull_pullback, bull_room, healthy_range, not_flat, not_overextended])
    bear_ready = all([bear_trend, bear_ema_slope, bear_structure, bear_dmi, strong_trend, bear_momentum, bear_rsi, bear_entry_trend, bear_entry_momentum, bear_entry_rsi, bear_candle, bear_pullback, bear_room, healthy_range, not_flat, not_overextended])
    signal = 'NO TRADE'
    ...
    if signal == 'NO TRADE':
        if bull_score >= bear_score:
            ...
            reasons = bear_reasons
    return {'symbol': symbol, 'signal': signal, 'score': int(score), 'price': price, 'rsi5': r5, 'rsi1': r1, 'adx': adx_now, 'structure': market_structure, 'extension_atr': extension, 'reasons': reasons, 'reason': 'Qualified setup' if signal != 'NO TRADE' else 'Filters not fully aligned'}
...
def add_new_signal(data, result, stamp):
    if result['signal'] == 'NO TRADE':
        return None
    signal_id = make_signal_id(result['symbol'], result['signal'], stamp)
    for item in data['signals']:
        if item.get('id') == signal_id:
            return None
    record = {'id': signal_id, 'symbol': result['symbol'], 'asset': result['symbol'], 'signal': result['signal'], 'score': result['score'], 'time': stamp.isoformat(), 'candle_time': stamp.isoformat(), 'price': result['price'], 'rsi5': result['rsi5'], 'rsi1': result['rsi1'], 'adx': result['adx'], 'structure': result['structure'], 'result': 'PENDING', 'result_time': None}
...
def build_report(results, errors, data):
    stamp = now_utc()
    qualified = [x for x in results if x['signal'] != 'NO TRADE']
    qualified.sort(key=lambda x: x['score'], reverse=True)
    lines = ['🧠 <b>PRECISION SCANNER V3.1</b>', '', 'Scan: ' + stamp.strftime('%Y-%m-%d %H:%M:%S UTC'), 'Data: Coinbase spot proxy', 'Trend: <b>5M</b>', 'Entry: <b>1M</b>', 'Reference expiry: <b>10 MINUTES</b>', 'Minimum setup score: <b>80/100</b>', '']
    if not qualified:
        lines += ['⚪ <b>NO TRADE</b>', 'No asset passed all precision filters.']
    else:
        lines.append('🔥 <b>QUALIFIED SIGNALS</b>')
        for result in qualified:
            signal_id = add_new_signal(data, result, stamp)
            if not signal_id:
                continue
            side = '🟢 CALL' if result['signal'] == 'CALL' else '🔴 PUT'
            lines += ['', '<b>' + html.escape(result['symbol']) + ' OTC proxy</b>', 'Signal: <b>' + side + '</b>', 'Setup quality: <b>' + str(result['score']) + '/100</b>', '5M RSI: %.1f' % result['rsi5'], '1M RSI: %.1f' % result['rsi1'], 'ADX: %.1f' % result['adx'], 'Price: %.8f' % result['price'], 'Signal ID: <code>' + html.escape(signal_id) + '</code>', 'Reference expiry: <b>10 MINUTES</b>']
            if result['reasons']:
                lines.append('Why: ' + html.escape(', '.join(result['reasons'][:5])))
    if errors:
        lines += ['', '⚠️ <b>MARKET/DATA WARNINGS</b>']
        for error in errors[:8]:
            lines.append('• ' + html.escape(error))
    lines += ['', 'Commands: /win ID | /loss ID | /stats', '', '⚠️ DEMO/TESTING ONLY.', 'Coinbase proxy may differ from Pocket Option OTC pricing.']
    return '\n'.join(lines)

def completed_signals(data):
    return [x for x in data['signals'] if x.get('result') in ('WIN', 'LOSS')]

def winrate(items):
    if not items:
        return 0.0
    wins = sum((x.get('result') == 'WIN' for x in items))
    return wins * 100.0 / len(items)

def stats_text(data):
    done = completed_signals(data)
    pending = sum((x.get('result') == 'PENDING' for x in data['signals']))
    wins = sum((x.get('result') == 'WIN' for x in done))
    losses = sum((x.get('result') == 'LOSS' for x in done))
    lines = ['📊 <b>PRECISION SCANNER V3.1</b>', '', 'Completed: ' + str(len(done)), 'Wins: ' + str(wins), 'Losses: ' + str(losses), 'Win rate: %.1f%%' % winrate(done), 'Pending: ' + str(pending), '']
    for side in ('CALL', 'PUT'):
        group = [x for x in done if x.get('signal') == side]
        lines.append(side + ': ' + str(sum((x.get('result') == 'WIN' for x in group))) + 'W / ' + str(sum((x.get('result') == 'LOSS' for x in group))) + 'L = %.1f%%' % winrate(group))
    lines += ['', '⚠️ Score is setup quality, not win probability.', 'Use forward testing before scaling.']
    return '\n'.join(lines)
...
def commit_tracker():
    try:
        ...
        subprocess.run(['git', 'add', TRACKER_FILE], check=True)
        check = subprocess.run(['git', 'diff', '--cached', '--quiet'])
        if check.returncode == 0:
            print('No tracker changes to commit.')
            return
        subprocess.run(['git', 'commit', '-m', 'Update scanner tracker'], check=True)
...
def main():
    ...
    for symbol in ASSET_BASES:
        ...
        if not product:
            errors.append(symbol + ': Coinbase market unavailable')
            continue
        print('Analyzing ' + symbol + ' (' + product + ')')
        try:
            result = analyze_asset(symbol, product)
            results.append(result)
            print(symbol + ': ' + result['signal'] + ' ' + str(result['score']) + '/100')
        except Exception as exc:
            errors.append(symbol + ': ' + str(exc))
            print(symbol + ': ERROR ' + str(exc))
    if not results:
        raise RuntimeError('All Coinbase markets failed.')
    message = build_report(results, errors, data)
    save_tracker(data)
